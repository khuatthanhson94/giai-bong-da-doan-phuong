...
const rapidRiceFile = path.join(__dirname, 'rapid_rice.db');
const buf = fs.readFileSync(rapidRiceFile);

console.log(`[Restore Scores] Loading ${buf.length} bytes database file with 15 finished group matches & 33 goals...`);

fs.writeFileSync(path.join(__dirname, 'tournament.db'), buf);
fs.writeFileSync(path.join(__dirname, '..', 'data', 'tournament.db'), buf);

console.log('✅ Overwrote local tournament.db with rapid-rice database containing finished match scores!');

const neonUrls = [
  'postgresql://neondb_owner:npg_BoQrt5haT7Fe@ep-rapid-rice-az2ir2s8-pooler.c-3.ap-southeast-1.aws.neon.tech/neondb?sslmode=require',
...
async function main() {
  for (const nUrl of neonUrls) {
    try {
      ...
      console.log(`📡 Uploading database with finished scores to Neon [${dbHost}]...`);
      ...
      console.log(`🎉 Successfully synced database to Neon [${dbHost}]!`);
      await client.end();
    } catch (e) {
