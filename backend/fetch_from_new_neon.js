...
const newNeonUrl = 'postgresql://neondb_owner:npg_fc6GZvtd2LJW@ep-little-butterfly-az3l3shn-pooler.c-3.ap-southeast-1.aws.neon.tech/neondb?sslmode=require';

async function main() {
  console.log('📡 Connecting to new Neon database (ep-little-butterfly-az3l3shn)...');
  const client = new Client({ connectionString: newNeonUrl });
  ...
  if (res.rows.length > 0 && res.rows[0].data) {
    ...
    console.log(`📥 Downloaded ${buf.length} bytes from ep-little-butterfly (Updated at: ${res.rows[0].updated_at})`);
