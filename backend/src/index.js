...
if (process.env.VERCEL) {
  app.use('/uploads/:filename', (req, res) => {
    const renderBackend = process.env.RENDER_BACKEND_URL || 'https://giai-bong-da-api-v4.onrender.com';
...
app.post('/api/admin/restore-database', upload.single('database'), async (req, res) => {
  ...
  try {
    const { reopenDatabase, dbPath } = await import('./db.js');
    
    // Reopen database using uploaded file safely
    reopenDatabase(req.file.path);
