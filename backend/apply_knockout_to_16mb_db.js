...
const file16MB = path.join(__dirname, 'soft_credit_download.db');
const dbPath1 = path.join(__dirname, 'tournament.db');
const dbPath2 = path.join(__dirname, '..', 'data', 'tournament.db');

const buf16 = fs.readFileSync(file16MB);
fs.writeFileSync(dbPath1, buf16);
fs.writeFileSync(dbPath2, buf16);

const db = new DatabaseSync(dbPath1);

// Make team_a_id & team_b_id nullable if needed
db.exec(`
...
// User requested Knockout matches dates/times
...
console.log('✅ Applied Bán kết & Chung kết schedule to 12 TDP database!');
