...
console.log('📡 Running matches table nullable migration on local database...');
...
try {
  ...
  `);
  db.exec('COMMIT');
  console.log('✅ Successfully migrated local matches table!');
} catch (err) {
  db.exec('ROLLBACK');
...
async function syncAll() {
  for (const nUrl of neonUrls) {
    try {
      ...
      console.log(`📡 Uploading migrated DB to Neon [${dbHost}]...`);
      ...
      console.log(`🎉 Successfully synced migrated DB to Neon [${dbHost}]!`);
      await client.end();
    } catch (e) {
