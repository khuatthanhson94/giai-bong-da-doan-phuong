...
async function main() {
  console.log('📡 Connecting to Neon branch (ep-spring-surf-az37ejml)...');
  ...
  if (res.rows.length === 0 || !res.rows[0].data) {
    console.log('⚠️ No sqlite_sync row in ep-spring-surf-az37ejml');
    await client.end();
    return;
  }

  const buf = res.rows[0].data;
  console.log(`📥 Downloaded ${buf.length} bytes from ep-spring-surf (Updated at: ${res.rows[0].updated_at})`);

  fs.writeFileSync('./backend/spring_surf_download.db', buf);
  fs.writeFileSync('./backend/tournament.db', buf);
  if (!fs.existsSync('./data')) fs.mkdirSync('./data', { recursive: true });
  fs.writeFileSync('./data/tournament.db', buf);

  console.log('✅ Overwrote local tournament.db with ep-spring-surf database!');

  const db = new DatabaseSync('./backend/spring_surf_download.db');
  ...
  // NOW SYNC THIS DATABASE ACROSS ALL NEON NODES!
  const neonUrls = [
    url,
  ...
  for (const nUrl of neonUrls) {
    try {
      ...
      console.log(`📡 Syncing DB from spring-surf to Neon [${dbHost}]...`);
      ...
      console.log(`🎉 Successfully synced DB to Neon [${dbHost}]!`);
      await nClient.end();
    } catch (e) {
